# app_week3.py
# Week 3 — Generative Poster (Streamlit, same logic structure)
# Keep the original logic: random palette -> blob function -> layered fill -> seed -> export.
...
# ---------------- Core logic (kept minimal & faithful) ----------------
...
def make_palette(k=8, mode=\"pastel\", base_h=0.60):
    \"\"\"Return k colors (RGB) using simple HSV sampling (pastel/vivid/mono/random).\"\"\"
    cols = []
    for _ in range(k):
        if mode == \"pastel\":
...
def draw_week3_poster(n_layers=12, wobble=0.12, palette_mode=\"pastel\", seed=None,
                      ...
                      figsize=(7,10), dpi=300, bg_gray=0.98, title=\"Generative Poster\",
                      subtitle=\"Week 3 • Made by Boyang Wang\"):
    ...
    # Labels (kept simple, like original scripts)
...
with st.sidebar:
    ...
    # Core controls — matching the Week 3 logic
    ...
    points = st.slider(\"Outline points (resolution)\", 50, 800, 200, 10)
    ...
    bg_gray = st.slider(\"Background gray (0-1)\", 0.90, 1.00, 0.98)
...
st.download_button(\"Download PNG\", data=png_buf.getvalue(),
                   file_name=\"week3_poster.png\", mime=\"image/png\")

pdf_buf = io.BytesIO()
fig.savefig(pdf_buf, format=\"pdf\", bbox_inches=\"tight\", pad_inches=0.1)
st.download_button(\"Download PDF\", data=pdf_buf.getvalue(),
                   file_name=\"week3_poster.pdf\", mime=\"application/pdf\")

svg_buf = io.BytesIO()
fig.savefig(svg_buf, format=\"svg\", bbox_inches=\"tight\", pad_inches=0.1)
st.download_button(\"Download SVG\", data=svg_buf.getvalue(),
                   file_name=\"week3_poster.svg\", mime=\"image/svg+xml\")
